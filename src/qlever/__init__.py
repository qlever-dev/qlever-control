...
def snake_to_camel(input_str: str) -> str:
    ...
    return "".join(
        [w.capitalize() for w in input_str.replace("-", "_").split("_")]
    )
...
def load_commands(package: str) -> dict[str, QleverCommand]:
    ...
    module name with `_` replaced by `-`. The names are sorted, so that the
    order in which the commands are shown does not depend on the file system.
    """
